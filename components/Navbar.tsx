...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    if (user && isLoaded) {
      console.log(
        "Client: user.publicMetadata (full object):",
        JSON.stringify(user.publicMetadata, null, 2)
      );
      console.log(
        "Client: user.publicMetadata.role =",
        (user.publicMetadata as CustomSessionClaims["metadata"])?.role
      );
    }
    ...
    <nav className="py-6 px-12 flex justify-between items-center rounded-lg shadow-md m-2 z-50 sticky">
                ...
                {searchResults.map((product) => (
                  ...
                  >
                    <Image
                      src={
                        product.imageUrls?.[0] ||
                        `https://placehold.co/30x30/F0F0F0/ADADAD?text=Img`
                      } // Use first image from imageUrls
                      alt={product.name}
                      width={30}
                      height={30}
                      ...
                      onError={(e) => {
                        ...
                        ).src = `https://placehold.co/30x30/F0F0F0/ADADAD?text=Img`;
                      }}
                    />
                    <Link
      ...
      <ul className="flex items-center space-x-4">
        <li>
          ...
          <Link
            href="/products"
            className="flex items-center gap-3 px-4 py-2 bg-white rounded-xl border border-none  hover:shadow-md transition duration-200"
          >
            Products
          </Link>
        </li>
        ...
        <SignedIn>
          ...
          <li>
            <Link
              href="/cart"
              className="relative flex items-center gap-3 px-4 py-2 bg-white rounded-xl border border-none  hover:shadow-md transition duration-200"
            >
